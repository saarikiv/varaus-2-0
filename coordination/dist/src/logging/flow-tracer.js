...
exports.FlowTracer = void 0;
exports.createFlowTracer = createFlowTracer;
/**
...
class FlowTracer {
     ...
     */
    addStage(correlationId, name, application, metadata) {
     ...
     */
    completeStage(correlationId, stageName, error) {
        const flow = this.activeFlows.get(correlationId);
        if (!flow) {
            this.logger.warn(`Flow not found for correlation ID: ${correlationId}`);
            return;
        ...
        const stage = [...flow.stages].reverse().find(s => s.name === stageName && s.status === 'pending');
        if (!stage) {
        ...
        stage.error = error;
        this.tracer.addEvent(correlationId, stage.application, `Stage ${error ? 'failed' : 'completed'}: ${stageName}`, stage.duration);
        this.logger.debug(`Flow stage ${error ? 'failed' : 'completed'}: ${stageName} (${stage.duration}ms)`, correlationId);
    }
    /**
     * End the flow and calculate timing breakdown
     */
    endFlow(correlationId) {
        const flow = this.activeFlows.get(correlationId);
        if (!flow) {
            this.logger.warn(`Flow not found for correlation ID: ${correlationId}`);
            return undefined;
        ...
        this.logger.info(`Flow trace completed (${flow.totalDuration}ms)`, correlationId);
        return flow;
    }
    /**
     * Get a flow by correlation ID
     */
    getFlow(correlationId) {
    ...
    formatTimeline(events) {
        ...
        const lines = ['Request Flow Timeline:', ''];
     ...
     */
    clearOldFlows(maxAgeMs = 3600000) {
        const now = Date.now();
        const toDelete = [];
    ...
    logRequestFlow(correlationId) {
        ...
        if (flow.totalDuration) {
            ...
            this.logger.info(`Timing Breakdown: Frontend=${breakdown.frontendProcessing}ms, ` +
                `Network=${breakdown.networkLatency}ms, ` +
                `Backend=${breakdown.backendProcessing}ms, ` +
                `Database=${breakdown.databaseOperations}ms, ` +
                `Total=${breakdown.totalRoundTrip}ms`, correlationId);
        }
    }
}
 ...
 */
function createFlowTracer(logger, tracer) {
