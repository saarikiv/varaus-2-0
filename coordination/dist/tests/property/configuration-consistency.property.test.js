...
describe('Property 4: Environment Configuration Consistency', () => {
    ...
    it('should ensure both applications use the same Firebase instance', async () => {
        await fc.assert(fc.asyncProperty(fc.record({
            ...
            try {
                ...
                (0, chai_1.expect)(config.frontend.firebaseConfig.projectId).to.equal(config.backend.firebaseConfig.projectId, 'Frontend and backend must use the same Firebase project ID');
                // Property: Both applications must use the same Firebase database
                (0, chai_1.expect)(config.frontend.firebaseConfig.databaseURL).to.equal(config.backend.firebaseConfig.databaseURL, 'Frontend and backend must use the same Firebase database URL');
                // Property: Firebase configs should be identical
                (0, chai_1.expect)(config.frontend.firebaseConfig).to.deep.equal(config.backend.firebaseConfig, 'Frontend and backend Firebase configurations must be identical');
            }
            catch (error) {
    ...
    it('should ensure frontend API endpoint matches backend configuration in development', async () => {
        await fc.assert(fc.asyncProperty(fc.record({
            ...
            try {
                ...
                (0, chai_1.expect)(config.frontend.apiEndpoint).to.include(config.backend.port.toString(), 'Frontend API endpoint should reference backend port in development');
            }
            catch (error) {
                if (error.message.includes('Missing required environment variable')) {
                    return; // Skip this test case
    ...
    it('should maintain configuration consistency across environment switches', async function () {
        ...
        await fc.assert(fc.asyncProperty(fc.record({
            environments: fc.array(fc.constantFrom('development', 'staging', 'production'), { minLength: 2, maxLength: 3 }),
            firebaseProjectId: fc.string({ minLength: 5, maxLength: 30 }),
            firebaseDatabaseURL: fc.webUrl(),
            apiKey: fc.string({ minLength: 20, maxLength: 50 }),
            authDomain: fc.webUrl(),
            ...
            for (const config of loadedConfigs) {
                (0, chai_1.expect)(config.frontend.firebaseConfig.projectId).to.equal(config.backend.firebaseConfig.projectId, `Configuration for ${config.shared.environment} should have matching Firebase project IDs`);
                (0, chai_1.expect)(config.frontend.firebaseConfig.databaseURL).to.equal(config.backend.firebaseConfig.databaseURL, `Configuration for ${config.shared.environment} should have matching Firebase database URLs`);
            }
        }), { numRuns: 100 });
    });
            ...
            try {
                ...
                if (testData.initialFirebaseProjectId !== testData.updatedFrontendProjectId) {
                    try {
                        ...
                        (0, chai_1.expect)(validationResult.valid).to.be.false;
                    }
                    catch (error) {
                        // Expected to throw due to validation failure
                        (0, chai_1.expect)(error.message).to.include('validation failed');
                    }
                }
            }
            catch (error) {
