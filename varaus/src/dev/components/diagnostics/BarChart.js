...
import { scaleBand as band, scaleLinear as linear, scaleTime, scaleOrdinal } from 'd3-scale';
import { select, max, schemeCategory10 } from 'd3';
...
import { timeParse as parse, timeFormat } from 'd3-time-format';
...
export default class BarChart extends React.Component {
  ...
  createYAxis2({ root, m, w, h }) {
    ...
    const axis = yAxisOrientRight ? axisLeft(y) : axisRight(y);
  ...
  createBarChart({ root, h, x, y }) {
    ...
    const mouseover = (event, d) => mouseOverHandler(d, event);
    const mouseout = (event, d) => mouseOutHandler(d, event);
    const mousemove = (event, d) => mouseMoveHandler(d, event);
    const click = (event, d) => clickHandler(d, event);
