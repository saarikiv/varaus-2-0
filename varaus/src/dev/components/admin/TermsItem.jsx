...
class TermsItem extends React.Component {
  ...
  remove(item){
    if(this.confirmation){
      this.props.actions.removeTermsItem(item);
    } else {
  ...
  toggleModify(item){
    if(this.toggleForm){
      ...
      this.props.actions.expandTermsForm(item.key)
    }    
  }

  renderForm(item){
    if(this.toggleForm){
      return ( <TermsForm mode="modify" dbKey={item.key} initialValues={item}/>)
    } else {
      return(<div></div>)
    }
  }
...
export default connect(mapStateToProps, mapDispatchToProps)(TermsItem)
